...
export async function generateMetadata({
  ...
  try {
    ...
    console.log("Received metadata response:", JSON.stringify(data).substring(0, 200) + "...");
    ...
    const title = fileData.title || `Sharing ${ownerProfile?.firstName ? `${ownerProfile.firstName}'s` : "A Typo* User's"} file with you`;
    
    // Clean up description
    const description = cleanDescription(fileData.description);
    
    // Get image for preview
    const imageUrl = fileData.image || 
      (textAttachment?.cloudFrontDownloadLink && /\.(jpg|jpeg|png|gif|webp)$/i.test(textAttachment.cloudFrontDownloadLink) ? 
    ...
    console.log(`Generated metadata - Title: "${title}", Description: "${description}"`);
    
    // Return complete metadata
    return {
      metadataBase: new URL(process.env.NEXT_PUBLIC_BASE_URL || "https://app.usetypo.com"),
      title,
      description,
      openGraph: {
        title,
