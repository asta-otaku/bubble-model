export function injectYTTheme(): HTMLTemplateElement {
  // SSR guard
  if (typeof window === "undefined") {
    return {} as HTMLTemplateElement;
  }

  let tpl = document.getElementById(
    "media-theme-yt"
  ) as HTMLTemplateElement | null;
  if (tpl) return tpl;

  tpl = document.createElement("template");
  tpl.id = "media-theme-yt";
  tpl.innerHTML = /* html */ `
          <style>
            media-controller {
              font-size: 13px;
          ...
          >
            ...
            <style>
              media-time-range {
                ...
                --media-range-bar-color: var(--media-accent-color, rgb(0, 25, 255));
                --media-range-thumb-border-radius: 13px;
                --media-range-thumb-background: var(--media-accent-color, rgb(0, 25, 255));
