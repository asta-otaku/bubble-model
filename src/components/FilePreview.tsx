...
function FilePreview({
  ...
  if (isVideo && fileUrl) {
    ...
    // Case 1: fileData structure passed from file-special page
    ...
    // Case 2: token is actually the textAttachment itself (from file-special)
    else if (token?.attachedContent?.muxPlaybackId) {
      muxPlaybackId = token.attachedContent.muxPlaybackId;
    }
    // Case 3: token is a normal message with content property
    else if (token.content?.muxPlaybackId) {
      muxPlaybackId = token.content.muxPlaybackId;
    } 
    // Case 4: token is a reference with nested content
